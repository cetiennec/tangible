...
@pause(prompt: "Turn q one and q two on the right, and watch the end-effector move.", speak: true)
...
@clear(kwIk)
@clear(ik)
For our two doff robot, we either have @cue(show.workspace = true) zero or two solutions in the general case, and sometimes one solution if we consider joint limits.
...
@clear(board)
In the two doff example, one can invert the equations of the FK with a bit of trigonometry, leading to the following @board(ikq2: $q_2 = \htmlClass{sign}{\pm}\arccos\left(\frac{x^2+y^2-L_1^2-L_2^2}{2L_1L_2}\right)$) @board(ikq1: $q_1 = \mathrm{atan2}(y,x) - \mathrm{atan2}(L_2\sin q_2,\, L_1+L_2\cos q_2)$) equations.
...
First, say hi to today's robot: it's an S O one oh one, designed by Hugging Face to be affordable and easy to record data with.
...
If we use a VR headset or the phone as a teleoperator, then we need IK. Indeed, they hand the robot a pose in the three D frame, and that has to be translated into joint angles before any motor can act on it.
...
But if we use another arm, the follower just has to copy-paste the joint angles directly, we say that planning is made in the joint space. To this purpose, HuggingFace also developed the S O one oh one leader arm, whose role is to be a teleoperator for the S O one oh one follower.
