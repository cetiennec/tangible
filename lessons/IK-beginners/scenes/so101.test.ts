...
describe("the SO-101 scene contract", () => {
  it("exposes one parameter per moving joint, plus the camera and the note", () => {
    expect(Object.keys(schema).sort()).toEqual(
      ["camera", "elbow", "gripper", "lift", "pan", "show.angles", "show.leader", "show.task", "task", "teleop", "wristFlex", "wristRoll"].sort(),
    );
  });
  ...
  it("has no separate joint parameters for the leader arm", () => {
    ...
    const jointish = Object.keys(schema).filter((k) => !["camera", "teleop", "show.leader", "show.angles", "task", "show.task"].includes(k));
