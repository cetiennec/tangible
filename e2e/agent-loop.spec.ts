...
test("agent loop: check catches error → fix → build → state + frame", () => {
  ...
  cpSync("e2e/fixtures/unit-circle/lesson.yaml", join(dir, "lesson.yaml"));
  cpSync("e2e/fixtures/unit-circle/scenes", join(dir, "scenes"), { recursive: true });
  cpSync("e2e/fixtures/unit-circle/assistant.md", join(dir, "assistant.md"));
